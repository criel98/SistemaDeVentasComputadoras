...
public class Menu extends javax.swing.JFrame {
    ...
    private void abrirFactura() {
        if (ventanaFactura == null || ventanaFactura.isClosed()) {
            ventanaFactura = new Factura();
            desktopPane.add(ventanaFactura);
            ventanaFactura.pack();

            // Centrar dentro del desktopPane
            int x = (desktopPane.getWidth() - ventanaFactura.getWidth()) / 2;
            int y = (desktopPane.getHeight() - ventanaFactura.getHeight()) / 2;
            ventanaFactura.setLocation(Math.max(x, 0), Math.max(y, 0));

            ventanaFactura.setVisible(true);
        } else {
            try {
                ventanaFactura.setSelected(true);
            } catch (java.beans.PropertyVetoException ex) {
                ex.printStackTrace();
            }
        }
